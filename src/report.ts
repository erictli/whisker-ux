...
export async function writeReport(
  ...
  const markdown = generateMarkdown(report, outputDir);
...
function generateMarkdown(report: WhiskerReport, outputDir: string): string {
  const lines: string[] = [];
  ...
  lines.push("");
  lines.push(
    `*Generated by Whisker v0.1.0 using ${report.metadata.modelUsed}*`
  );
